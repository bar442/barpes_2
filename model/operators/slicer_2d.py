...
class Slicer_2D():
    def __init__(self,data:np.ndarray , 
                ...
                viewed_index_0:int=0 ,
                viewed_index_1:int=1 ,
        ...
        self._viewed_index_0:int=viewed_index_0
        self._viewed_index_1:int=viewed_index_1
        ...
        self._shifts:List[float] = shifts if shifts else [0 for i in range(len(self._data.shape))]
        self._normalization_options:List[str]=["None","Volume","row-by-row","col-by-col"]
        self._normalization_mode:str = "None"

        # -----------> Events & Subscriptions 
        self._subscribers_to_redraw_needed:List[Callable[[Slicer_2D], None]]=[]
        self._subscribers_to_viewed_indices_changed:List[Callable[[Slicer_2D], None]]=[]


    # region Main Properties
    @property
    def sliced_data(self)->np.ndarray:
        if self._viewed_index_0==self._viewed_index_1: raise Exception("Slicer_2D cannot slice to identical viewed indices")

        ####  --- build Slicer
        slicer_list = []
        for dim_i in range(len(self._data.shape)):
            if dim_i == self._viewed_index_0 or dim_i == self._viewed_index_1:
                slicer_list.append(slice(None))
            else:
                start_j= self._slice_indices[dim_i] - int(math.floor(self._integration_widths[dim_i]/2))
                finish_j= self._slice_indices[dim_i]  + 1 + int(math.floor(self._integration_widths[dim_i]/2))
        ...
        ####  --- initial slice
        result = np.sum(self._data[tuple(slicer_list)], axis=tuple(ax for ax in range(len(self._data.shape)) if ax not in [self._viewed_index_0,self._viewed_index_1]))

        ####  --- reorder indices if needed
        if self._viewed_index_0 > self._viewed_index_1:
            result = np.moveaxis(result,(0,1),(1,0))
    ...
    @property
    def image(self)->np.ndarray:return self.sliced_data

    @property
    def viewed_scale_0(self)->np.ndarray: return self._scales[self._viewed_index_0] + self._shifts[self._viewed_index_0]

    @property
    def viewed_scale_1(self)->np.ndarray: return self._scales[self._viewed_index_1] + self._shifts[self._viewed_index_1]

    @property
    def viewed_scales(self)->List[np.ndarray]: return [self.viewed_scale_0,self.viewed_scale_1] 

    @property
    def viewed_scales_labels(self)->List[np.ndarray]: return [self._scales_labels[self._viewed_index_0],self._scales_labels[self._viewed_index_1]] 
    ...
    def setViewedIndex_0(self,value:int):
        if self._viewed_index_0 != value:
            if value == self._viewed_index_1:
                self._viewed_index_1=self._viewed_index_0
            self._viewed_index_0=value
            self.emit_viewed_indices_changed()
            self.emit_redraw_needed()

    def setViewedIndex_1(self,value:int):
        if self._viewed_index_1 != value:
            if value == self._viewed_index_0:
                self._viewed_index_0=self._viewed_index_1
            self._viewed_index_1=value
            self.emit_viewed_indices_changed()
            self.emit_redraw_needed()
    ...
    def setShift(self,axis_index:int,value:float):
        if axis_index > len(self._scales):raise Exception("axis_index out of range")
        if value != self._shifts[axis_index]:
            self._shifts[axis_index]=value
            self.emit_redraw_needed()
            self.emit_viewed_indices_changed()

    def setAllShifts(self,values:List[float]):
        for ai,v in enumerate(values):
            if ai > len(self._scales):raise Exception("axis_index out of range")
    ...
    def emit_redraw_needed(self):
        for f in self._subscribers_to_redraw_needed:
            f(self)
    def subscribe_to_redraw_needed(self,func:Callable[['Slicer_2D'], None]):
        self._subscribers_to_redraw_needed.append(func)

    def emit_viewed_indices_changed(self):
        for f in self._subscribers_to_viewed_indices_changed:
            f(self)
    def subscribe_to_viewed_indices_changed(self,func:Callable[['Slicer_2D'], None]):
        self._subscribers_to_viewed_indices_changed.append(func)
